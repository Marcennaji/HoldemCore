...
class OpponentsStrengthsEvaluatorTest : public EngineTest
{
  protected:
    void SetUp() override
    {
        ...
        m_evaluator = std::make_unique<OpponentsStrengthsEvaluator>(
            0, // player ID
            getLogger(),
            *getHandEvaluationEngineService()
        );
    }
     ...
     */
    void setupBasicScenario(const HoleCards& heroCards, 
                           const std::optional<HoleCards>& opponent1Cards = std::nullopt,
                           ...
                           const std::optional<BoardCards>& communityCards = std::nullopt)
    {
        ...
        if (opponent1Cards.has_value()) requiredPlayerCount = std::max(requiredPlayerCount, 2);
        if (opponent2Cards.has_value()) requiredPlayerCount = std::max(requiredPlayerCount, 3);
        ...
        heroPlayer->setAction(currentHandState, PlayerAction{heroPlayer->getId(), ActionType::Call, standardCallAmount});
        
        // Record that first opponent called if they're in the hand
        if (opponent1Cards.has_value() && requiredPlayerCount >= 2)
        {
            auto firstOpponent = getPlayerById(1);
            firstOpponent->setAction(currentHandState, PlayerAction{firstOpponent->getId(), ActionType::Call, standardCallAmount});
        }
        ...
        {
            auto secondOpponent = getPlayerById(2);
            secondOpponent->setAction(currentHandState, PlayerAction{secondOpponent->getId(), ActionType::Call, standardCallAmount});
        }
...
{
    auto evaluator1 = std::make_unique<OpponentsStrengthsEvaluator>(
        0, getLogger(), *getHandEvaluationEngineService());
    auto evaluator2 = std::make_unique<OpponentsStrengthsEvaluator>(
        1, getLogger(), *getHandEvaluationEngineService());
...
TEST_F(OpponentsStrengthsEvaluatorTest, SingleOpponent_WithWeakerHand_ReturnsLowStrength)
{
    ...
    auto result = m_evaluator->evaluateOpponents(
        evaluatingPlayer->getCurrentHandContext(),
        evaluatingPlayer->getHoleCards(),
...
{
    setupBasicScenario(HoleCards("Ts", "Tc"), HoleCards("7c", "6c"), HoleCards("As", "Ah"), BoardCards("Kc", "8d", "Qc"));
...
{
    setupBasicScenario(HoleCards("As", "Ah"), HoleCards("7c", "6c"), HoleCards("9s", "8s"), BoardCards("Kc", "8d", "Qc"));
...
{
    setupBasicScenario(HoleCards("Jc", "Jh"), HoleCards("7c", "6c"), HoleCards("As", "Ah"), BoardCards("Kc", "8d", "Qc"));
...
TEST_F(OpponentsStrengthsEvaluatorTest, OpponentWithEmptyRange_ComputesRangeAutomatically)
{
    setupBasicScenario(HoleCards("As", "Ah"), HoleCards("7c", "6c"), std::nullopt, BoardCards("Kc", "8d", "Qc"));
    ...
    auto player1 = getPlayerById(1);
    // Don't set range - evaluator should handle it
    EXPECT_TRUE(player1->getRangeEstimator()->getEstimatedRange().empty());
    ...
    // Should still evaluate (range computed automatically)
    EXPECT_EQ(result.opponentStrengths.size(), 1);
    EXPECT_GE(result.maxStrength, 0.0f);
    EXPECT_LE(result.maxStrength, 1.0f);
}
...
{
    setupBasicScenario(HoleCards("As", "Ah"), HoleCards("7c", "6c"), HoleCards("Kc", "Kd"), BoardCards("2h", "3h", "4h"));
...
{
    setupBasicScenario(HoleCards("Tc", "Th"), HoleCards("7c", "6c"), HoleCards("Qc", "Qd"), BoardCards("Kc", "8d", "Qc"));
...
TEST_F(OpponentsStrengthsEvaluatorTest, IntegrationWithPlayer_EvaluatesCorrectly)
{
    ...
    auto result = playerEvaluator->evaluateOpponents(
        evaluatingPlayer->getCurrentHandContext(),
        evaluatingPlayer->getHoleCards(),
        evaluatingPlayer->getHandRanking()
    );
...
TEST_F(OpponentsStrengthsEvaluatorTest, MultipleEvaluations_ProduceConsistentResults)
{
    ...
    // Evaluate twice
    auto result1 = m_evaluator->evaluateOpponents(
        evaluatingPlayer->getCurrentHandContext(),
        evaluatingPlayer->getHoleCards(),
        evaluatingPlayer->getHandRanking()
    );
    
    auto result2 = m_evaluator->evaluateOpponents(
        evaluatingPlayer->getCurrentHandContext(),
        evaluatingPlayer->getHoleCards(),
        evaluatingPlayer->getHandRanking()
    );
