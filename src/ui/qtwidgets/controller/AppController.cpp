...
AppController::AppController(std::shared_ptr<pkt::core::Logger> logger,
    ...
    : m_logger(std::move(logger)), m_handEvaluationEngine(std::move(handEvaluationEngine)),
      m_statisticsStore(std::move(statisticsStore)), m_randomizer(std::move(randomizer))
{
    ...
    m_session = std::make_unique<pkt::core::Session>(m_events, *m_engineFactory, *m_logger, *m_handEvaluationEngine,
                                                     *m_statisticsStore, *m_randomizer);
    m_pokerTableWindow = std::make_unique<pkt::ui::qtwidgets::PokerTableWindow>(m_session.get());
    m_bridge = std::make_unique<Bridge>(m_session.get(), m_pokerTableWindow.get());

    m_bridge->connectEventsToUi(m_events);
}

pkt::ui::qtwidgets::StartWindow* AppController::createMainWindow()
{
    auto* w = new pkt::ui::qtwidgets::StartWindow(m_pokerTableWindow.get(), m_session.get(), nullptr);
    if (m_pokerTableWindow)
        m_pokerTableWindow->hide();
    return w;
}
