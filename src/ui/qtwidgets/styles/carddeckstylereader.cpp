...
void CardDeckStyleReader::readStyleFile(QString file, const string& appDataDir)
{
    ...
    if (doc.RootElement())
    {
        ...
        if (GameTableElement)
        {
            // LOG_ERROR(__FILE__ << " (" << __LINE__ << "A game table style was selected instead of a card deck
        ...
        else
        {
            ...
            TiXmlElement* itemsList =
                docHandle.FirstChild("PokerTraining").FirstChild("CardDeck").FirstChild().ToElement();
            ...
            if (leftItems.isEmpty() && cardsLeft.isEmpty() && PokerTrainingStyleFileVersion != "" &&
                PokerTrainingStyleFileVersion.toInt() == POKERTRAINING_CD_STYLE_FILE_VERSION)
                loadedSuccessfull = 1;
            else
                loadedSuccessfull = 0;

            if (!leftItems.isEmpty() && myW != 0)
                showLeftItemsErrorMessage(StyleDescription, leftItems, StyleMaintainerEMail);
            else
            {
