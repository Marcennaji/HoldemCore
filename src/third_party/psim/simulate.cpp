...
#include <third_party/poker-eval/lib/t_nbitsandstr.c>
...
unsigned int BoardsToEnumerate(unsigned int cardsToDraw)
{
    ...
    {
    case 5:
        return 2118760; // 50 choose 5
    case 4:
        return 211876; // 49 choose 4
    case 3:
        return 17296; // 48 choose 3
    case 2:
        return 1081; // 47 choose 2
    case 1:
        return 46; // 46 choose 1
    default:
        return 0;
    }
}
...
void GetHandState(const Hand& shand, PostFlopState* state)
{
    ...
    { // Grab some info about our draws.
        ...
        ENUMERATE_N_CARDS_D(next, 1, shand.dead, {
            ...
            CardMask totalDead;
            CardMask_OR(totalDead, hand, next);
            CardMask oppCards;
            ENUMERATE_N_CARDS_D(oppCards, 2, totalDead, {
                // Grab what the opponent could have right now.
                CardMask oppHand;
                CardMask_OR(oppHand, oppCards, shand.common);

                drawnType = Hand_EVAL_TYPE(oppHand, shand.size);
                if (drawnType == StdRules_HandType_STRAIGHT)
                    state->IsStraightPossible = true;
                if (drawnType == StdRules_HandType_FLUSH)
                    state->IsFlushPossible = true;

                // Now consider what the opponent could draw.
                CardMask_OR(oppHand, oppHand, next);
                drawnType = Hand_EVAL_TYPE(oppHand, size);
                if (drawnType == StdRules_HandType_STRAIGHT)
                    state->IsStraightDrawPossible = true;
                if (drawnType == StdRules_HandType_FLUSH)
                    state->IsFlushDrawPossible = true;
            });
        });
    }
}
...
void SimCurrent(const Hand& shand, SimResults* results, float lowRange, float highRange)
{
    ...
    if (EnumRange)
        HostileEnumRange(SimCurrentLoop) else HostileEnumAll(SimCurrentLoop)
...
void SimSd(const Hand& shand, SimResults* results, float lowRange, float highRange, size_t uboards)
{
    ...
    // NOTE THAT WE ENUMERATE THE BOARDS BEFORE THE OPPONENT CARDS, IF DONE OTHERWISE IT'LL TAKE TWICE THE EVALUATIONS!!
    // (our way we only do our hand once for each board, the other way you must do our hand once for each opponent *
    // each board)
    ...
    { // MonteCralo for the number of boards.
        if (EnumRange)
            DECK_MONTECARLO_N_CARDS_D(StdDeck, commonDraw, dead, cardsToDraw, boards,
                                      {SimSdInnerLoopRange(HostileEnumRange)});
        else
            DECK_MONTECARLO_N_CARDS_D(StdDeck, commonDraw, dead, cardsToDraw, boards,
                                      {SimSdInnerLoopRange(HostileEnumAll)});
    }
    else
