...
Player::Player(GameEvents* events, IHandAuditStore* ha, IPlayersStatisticsStore* ps, int id, PlayerType type,
    ...
    : myHandAuditStore(ha), myPlayersStatisticsStore(ps), currentHand(0), myID(id), myType(type), myName(name),
      ...
      myEvents(events)
{
    myRangeManager = std::make_unique<RangeManager>(myID, currentHand, myPlayersStatisticsStore);
    loadStatistics();
...
void Player::updateUnplausibleRangesGivenFlopActions()
{
    ...
    myRangeManager->setEstimatedRange(
        substractRange(myRangeManager->getEstimatedRange(), unplausibleRanges, stringBoard));
                                   ...
                                   const std::string board)
{
    ...
    while (getline(oss, singleOriginRange, ','))
    {
        ...
        vector<std::string> cardsInOriginRange = myRangeManager->getRangeAtomicValues(
             ...
             originHand != cardsInOriginRange.end(); originHand++)
        {
            ...
            if (isCardsInRange(originCard1, originCard2, rangesToSubstract))
            {
                ...
                vector<std::string> atomicRangesInSingleOriginRange =
                    myRangeManager->getRangeAtomicValues(singleOriginRange, true);
                     ...
                     atomicOriginRange != atomicRangesInSingleOriginRange.end(); atomicOriginRange++)
                {
                    ...
                    if (originCard1.at(1) == originCard2.at(1))
                    {
                        ...
                        vector<std::string> handsInAtomicRange =
                            myRangeManager->getRangeAtomicValues(*atomicOriginRange);
