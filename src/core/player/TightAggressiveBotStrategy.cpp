...
TightAggressiveBotStrategy::TightAggressiveBotStrategy(GameEvents* events, IHandAuditStore* ha,
                                                       IPlayersStatisticsStore* ps, int id, PlayerType type,
...
bool TightAggressiveBotStrategy::preflopShouldCall()
{
    ...
    if (currentHand->getRunningPlayerList()->size() > 2 && nbRaises + nbCalls > 1 && nbRaises == 1 &&
        myPosition >= LATE && getCash() >= currentHand->getBoard()->getPot() * 10 && lastRaiser != NULL &&
    ...
    if (nbRaises == 2 && myCurrentHandActions.getPreflopActions().size() > 0 &&
        myCurrentHandActions.getPreflopActions().back() == PLAYER_ACTION_RAISE &&
        getCash() >= currentHand->getBoard()->getPot() * 10 && lastRaiser != NULL &&
...
#ifdef LOG_POKER_EXEC
            cout << "\t\TAG defending against 3-bet : adding high suited connectors, high suited aces and pairs to the "
                    "initial calling range."
                 << endl;
#endif
        }
    }
#ifdef LOG_POKER_EXEC
    cout << "\t\tTAG final calling range is " << stringCallingRange << endl;
#endif

    return isCardsInRange(myCard1, myCard2, stringCallingRange);
}

bool TightAggressiveBotStrategy::preflopShouldRaise()
{
    ...
    if (nbRaises == 1)
    {
            ...
            nbCalls < 2)
        {
            ...
            if (canBluff(GAME_STATE_PREFLOP) && myPosition > LATE &&
            ...
            else
            {
                    ...
                    raiserStats.getPreflopCall3BetsFrequency() < 30)
                {
                    ...
                    speculativeHandedAdded = true;
#ifdef LOG_POKER_EXEC
                    cout << "\t\tTAG adding this speculative hand to our initial raising range";
#endif
                }
            }
        }
    ...
    if (!speculativeHandedAdded && currentHand->getPreflopCallsNumber() == 0 &&
        currentHand->getPreflopRaisesNumber() == 1 && raisingRange < 100 &&
...
bool TightAggressiveBotStrategy::flopShouldBet()
{
    ...
    if (nbPreflopRaises > 0 && lastRaiserID != myID)
    {
        ...
        if (lastRaiser->getPosition() > myPosition)
        {
            ...
            if ((myFlopState.IsTwoPair || myFlopState.IsTrips || myFlopState.IsStraight) &&
            ...
            if (nbPlayers < 3 && canBluff(GAME_STATE_FLOP) && getBoardCardsHigherThan("Jh") < 2 &&
                getBoardCardsHigherThan("Kh") == 0 && !myFlopState.IsFlushDrawPossible)
            {
...
bool TightAggressiveBotStrategy::flopShouldCall()
{
    ...
    if (myFlopHandSimulation.winRanged * 100 < getPotOdd() && myFlopHandSimulation.win < 0.94)
        return false;
...
bool TightAggressiveBotStrategy::flopShouldRaise()
{
    ...
    if ((isDrawingProbOk() || bHavePosition) && runningPlayers->size() == 2 &&
        !(myFlopHandSimulation.winRanged * 100 < getPotOdd()) && canBluff(GAME_STATE_FLOP) && nbRaises < 2)
    {
    ...
    if (myFlopHandSimulation.winRanged * 100 < getPotOdd())
    {
        ...
        if (getPotOdd() < 30 && runningPlayers->size() < 4)
        {
            ...
            if (rand == 2 && myFlopHandSimulation.winRanged > 0.3 && myFlopHandSimulation.win > 0.3)
            {
                myRaiseAmount = pot;
                return true;
            }
    ...
    if (myFlopHandSimulation.winRanged > 0.9 && nbRaises < 3)
    {
        myRaiseAmount = pot;
        return true;
    }
    if (myFlopHandSimulation.winRanged > 0.75 && nbRaises < 2)
    {
        myRaiseAmount = pot;
        return true;
    }
...
bool TightAggressiveBotStrategy::turnShouldBet()
{
    ...
    if (currentHand->getFlopBetsOrRaisesNumber() > 1 && !isAgressor(GAME_STATE_FLOP) &&
        myTurnHandSimulation.winRanged < 0.75 && myTurnHandSimulation.win < 0.9)
        return false;
    ...
    if (myCash < pot * 4 && myTurnHandSimulation.winRanged < 0.6 && myTurnHandSimulation.win < 0.9)
        return false;

    if (myTurnHandSimulation.winRanged < 0.6 && myTurnHandSimulation.win < 0.9 && !bHavePosition)
        return false;

    if (myTurnHandSimulation.winRanged > 0.5 && myTurnHandSimulation.win > 0.7 && bHavePosition)
    {
        myBetAmount = pot * 0.6;
        return true;
    }
...
bool TightAggressiveBotStrategy::turnShouldCall()
{
    ...
    if (nbRaises == 2 && myTurnHandSimulation.winRanged < 0.8)
    {
    ...
    if (nbRaises > 2 && myTurnHandSimulation.winRanged < 0.9)
    {
        if (raiserStats.m_hands <= MIN_HANDS_STATISTICS_ACCURATE)
            return false;
        if (raiserStats.getAgressionFrequency() < 20)
    ...
    if (myTurnHandSimulation.winRanged < 0.25)
        return false;

    return true;
}

bool TightAggressiveBotStrategy::turnShouldRaise()
{
...
bool TightAggressiveBotStrategy::riverShouldBet()
{
    ...
    // bluff if very few chances to win, and if I was the agressor on turn
    if (isAgressor(GAME_STATE_TURN))
    {

        if (myRiverHandSimulation.winRanged < .15 && myRiverHandSimulation.winSd > 0.3 && runningPlayers->size() < 4 &&
    ...
    if (myRiverHandSimulation.winSd > 0.5 &&
        (myRiverHandSimulation.winRanged > .9 || (bHavePosition && myRiverHandSimulation.winRanged > .8)))
    {
        int rand = 0;
        Randomizer::GetRand(1, 3, 1, &rand);
        if (rand == 1 || bHavePosition)
    ...
    if (bHavePosition && nbRaises == 0 && myRiverHandSimulation.winRanged < 0.3 &&
    ...
    if (bHavePosition && nbRaises == 0 && myRiverHandSimulation.winRanged > 0.7 &&
        currentHand->getTurnBetsOrRaisesNumber() == 0)
    {
        int rand = 0;
        Randomizer::GetRand(1, 3, 1, &rand);
...
bool TightAggressiveBotStrategy::riverShouldCall()
{
    ...
    if (myRiverHandSimulation.winRanged * 100 < getPotOdd() && myRiverHandSimulation.winRanged < 0.94)
    {
        if (raiserStats.m_hands > MIN_HANDS_STATISTICS_ACCURATE && raiserStats.getAgressionFrequency() < 40)
            return false;
    }

    if (myRiverHandSimulation.winRanged < .7 && nbRaises == 1)
    {
        if (raiserStats.m_hands > MIN_HANDS_STATISTICS_ACCURATE &&
    ...
    if (myRiverHandSimulation.winRanged < .9 && nbRaises > 1)
    {
        if (raiserStats.m_hands > MIN_HANDS_STATISTICS_ACCURATE &&
            lastRaiser->getStatistics(nbPlayers).getWentToShowDown() < 40)
            return false;
    ...
    if (getPotOdd() > 10 && myRiverHandSimulation.winRanged < .5 &&
...
bool TightAggressiveBotStrategy::riverShouldRaise()
{
    ...
    if (nbRaises < 2 && myRiverHandSimulation.winRanged * 100 > getPotOdd() && myRiverHandSimulation.winRanged > 0.9 &&
        myRiverHandSimulation.winSd > 0.5)
    {
