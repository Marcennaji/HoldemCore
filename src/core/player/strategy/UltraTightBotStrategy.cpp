...
UltraTightBotStrategy::UltraTightBotStrategy()
{
    ...
    myServices->randomizer().getRand(1, 2, 1, &utgFullTableRange);
...
int UltraTightBotStrategy::flopShouldRaise(const CurrentHandContext& ctx)
{
    ...
    if (ctx.personalContext.myHandSimulation.winRanged > 0.93 && ctx.personalContext.myHandSimulation.win > 0.5 &&
        nbRaises < 3)
    {
        return ctx.commonContext.bettingContext.pot;
    }
...
int UltraTightBotStrategy::turnShouldBet(const CurrentHandContext& ctx)
{
    ...
    if (ctx.commonContext.bettingContext.flopBetsOrRaisesNumber > 1 && !ctx.personalContext.actions.flopIsAggressor &&
        ctx.personalContext.myHandSimulation.winRanged < 0.7 && ctx.personalContext.myHandSimulation.win < 0.9)
    {
        return 0;
    }
    ...
    if (ctx.personalContext.myHandSimulation.winRanged < 0.7 && ctx.personalContext.myHandSimulation.win < 0.9 &&
        !ctx.personalContext.hasPosition)
    {
        return 0;
    }
...
int UltraTightBotStrategy::riverShouldBet(const CurrentHandContext& ctx)
{
    ...
    if (!ctx.personalContext.hasPosition && ctx.personalContext.myHandSimulation.winRanged < 0.8 &&
        ctx.personalContext.myHandSimulation.winRanged > 0.6 && ctx.personalContext.myHandSimulation.winSd > 0.4)
    {
        int rand = 0;
        myServices->randomizer().getRand(1, 2, 1, &rand);
        if (rand == 1)
    ...
    if (ctx.personalContext.myHandSimulation.winRanged > .85 ||
        (ctx.personalContext.hasPosition && ctx.personalContext.myHandSimulation.winRanged > .75) &&
            ctx.personalContext.myHandSimulation.winSd > 0.5)
    {
        int rand = 0;
        myServices->randomizer().getRand(1, 7, 1, &rand);
        if (rand != 1 || ctx.personalContext.hasPosition)
    ...
    if (ctx.personalContext.hasPosition && ctx.commonContext.bettingContext.riverBetsOrRaisesNumber == 0 &&
        ctx.personalContext.myHandSimulation.winRanged > 0.8 && ctx.personalContext.myHandSimulation.winSd > 0.5 &&
...
bool UltraTightBotStrategy::riverShouldCall(const CurrentHandContext& ctx)
{
    ...
    if (ctx.personalContext.myHandSimulation.winRanged < .6 && ctx.personalContext.myHandSimulation.winSd < 0.97 &&
        nbRaises == 1)
    {
    ...
    if (ctx.personalContext.myHandSimulation.winRanged < .8 && ctx.personalContext.myHandSimulation.winSd < 0.97 &&
        nbRaises > 1)
    {
    ...
    if (ctx.commonContext.bettingContext.potOdd > 10 && ctx.personalContext.myHandSimulation.winRanged < .5 &&
        ctx.personalContext.myHandSimulation.winSd < 0.8 &&
