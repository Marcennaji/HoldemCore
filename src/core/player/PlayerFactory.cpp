...
PlayerFactory::PlayerFactory(const GameEvents& events, StrategyAssigner* assigner, pkt::core::Logger& logger,
                             pkt::core::HandEvaluationEngine& handEvaluator,
                             pkt::core::PlayersStatisticsStore& statisticsStore, pkt::core::Randomizer& randomizer)
    : m_events(events), m_strategyAssigner(assigner), m_logger(logger), m_handEvaluator(handEvaluator),
...
std::shared_ptr<Player> PlayerFactory::createHumanPlayer(int id, int startMoney)
{
    ...
    auto player = std::make_shared<Player>(m_events, m_logger, m_handEvaluator, m_statisticsStore, m_randomizer, id,
                                           "Human", startMoney);
...
std::shared_ptr<Player> PlayerFactory::createBotPlayer(int id, TableProfile profile, int startMoney)
{
    ...
    auto player = std::make_shared<Player>(m_events, m_logger, m_handEvaluator, m_statisticsStore, m_randomizer, id,
                                           "Bot_" + std::to_string(id), startMoney);
